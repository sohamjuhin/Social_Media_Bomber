...
""" 4 Step: 
    cautain: be sure your connection is fast and takes no more than 4-5 seconds to enter the victim's username as Chrome driver is stateless
             and opening a new URL after an interval of seconds may log you out of your Twitter account.
    
    1) run command python Twitter_bomber.py -u *your_twitter_username* -p *your_twitter_password*
    2) enter the correct username of the person you want to spam, make sure their Twitter messages are open, after the message screen is opened, 
       minimize the window and follow step #3
    3) press 1 if you want to manually type the message you want to send, or 2 if you want to import a file and extract the message from there
    4) enter the message count

"""

parser.add_argument('-u', '--username', type=str, required=True, help="Twitter username @ of user, can be their phone number or email")
...
def attack():
    ...
    name = input('Enter the Twitter name of the victim: ')
    if len(name) >= 3:
        pass
    else:
        name = input('Enter the name of the victim correctly: ')
